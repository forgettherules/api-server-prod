...
const cacheable = new cacheable_lookup_1.default();
// Install cacheable lookup for all other requests
cacheable.install(node_http_1.default.globalAgent);
cacheable.install(node_https_1.default.globalAgent);
const ws = (0, express_ws_1.default)((0, express_1.default)());
const app = ws.app;
...
const { addQueue, removeQueue, setQueues, replaceQueues } = createBullBoard({
    queues: [new BullAdapter((0, queue_service_1.getScrapeQueue)())],
...
function startServer(port = DEFAULT_PORT) {
    const server = app.listen(Number(port), HOST, () => {
        ...
        logger_1.logger.info(`For the Queue UI, open: http://${HOST}:${port}/admin/${process.env.BULL_AUTH_KEY}/queues`);
    });
    const exitHandler = () => {
...
app.use((err, req, res, next) => {
    ...
    logger_1.logger.error("Error occurred in request! (" + req.path + ") -- ID " + id + " -- " + verbose);
