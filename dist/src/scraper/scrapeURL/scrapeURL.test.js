...
describe("Standalone scrapeURL tests", () => {
    describe.each(testEngines)("Engine %s", (forceEngine) => {
        it("Basic scrape", async () => {
            const out = await (0, _1.scrapeURL)("test:scrape-basic", "https://www.roastmywebsite.ai/", types_1.scrapeOptions.parse({}), { forceEngine });
        ...
        it("Scrape of a page with 400 status code", async () => {
            const out = await (0, _1.scrapeURL)("test:scrape-400", "https://httpstat.us/400", types_1.scrapeOptions.parse({}), { forceEngine });
            // expect(out.logs.length).toBeGreaterThan(0);
            expect(out.success).toBe(true);
            if (out.success) {
                expect(out.document.warning).toBeUndefined();
        ...
        it("Scrape of a page with 401 status code", async () => {
            const out = await (0, _1.scrapeURL)("test:scrape-401", "https://httpstat.us/401", types_1.scrapeOptions.parse({}), { forceEngine });
            // expect(out.logs.length).toBeGreaterThan(0);
            expect(out.success).toBe(true);
            if (out.success) {
                expect(out.document.warning).toBeUndefined();
        ...
        it("Scrape of a page with 403 status code", async () => {
            const out = await (0, _1.scrapeURL)("test:scrape-403", "https://httpstat.us/403", types_1.scrapeOptions.parse({}), { forceEngine });
            // expect(out.logs.length).toBeGreaterThan(0);
            expect(out.success).toBe(true);
            if (out.success) {
                expect(out.document.warning).toBeUndefined();
        ...
        it("Scrape of a page with 404 status code", async () => {
            const out = await (0, _1.scrapeURL)("test:scrape-404", "https://httpstat.us/404", types_1.scrapeOptions.parse({}), { forceEngine });
            // expect(out.logs.length).toBeGreaterThan(0);
            expect(out.success).toBe(true);
            if (out.success) {
                expect(out.document.warning).toBeUndefined();
        ...
        it("Scrape of a page with 405 status code", async () => {
            const out = await (0, _1.scrapeURL)("test:scrape-405", "https://httpstat.us/405", types_1.scrapeOptions.parse({}), { forceEngine });
            // expect(out.logs.length).toBeGreaterThan(0);
            expect(out.success).toBe(true);
            if (out.success) {
                expect(out.document.warning).toBeUndefined();
        ...
        it("Scrape of a page with 500 status code", async () => {
            const out = await (0, _1.scrapeURL)("test:scrape-500", "https://httpstat.us/500", types_1.scrapeOptions.parse({}), { forceEngine });
            // expect(out.logs.length).toBeGreaterThan(0);
            expect(out.success).toBe(true);
            if (out.success) {
                expect(out.document.warning).toBeUndefined();
        ...
        it("Scrape a redirected page", async () => {
            const out = await (0, _1.scrapeURL)("test:scrape-redirect", "https://scrapethissite.com/", types_1.scrapeOptions.parse({}), { forceEngine });
            // expect(out.logs.length).toBeGreaterThan(0);
            expect(out.success).toBe(true);
            if (out.success) {
                expect(out.document.warning).toBeUndefined();
    ...
    it("Scrape of a PDF file", async () => {
        const out = await (0, _1.scrapeURL)("test:scrape-pdf", "https://arxiv.org/pdf/astro-ph/9301001.pdf", types_1.scrapeOptions.parse({}));
        // expect(out.logs.length).toBeGreaterThan(0);
        expect(out.success).toBe(true);
        if (out.success) {
            expect(out.document.warning).toBeUndefined();
    ...
    it("Scrape a DOCX file", async () => {
        const out = await (0, _1.scrapeURL)("test:scrape-docx", "https://nvca.org/wp-content/uploads/2019/06/NVCA-Model-Document-Stock-Purchase-Agreement.docx", types_1.scrapeOptions.parse({}));
        // expect(out.logs.length).toBeGreaterThan(0);
        expect(out.success).toBe(true);
        if (out.success) {
            expect(out.document.warning).toBeUndefined();
    ...
    test.concurrent.each(new Array(100).fill(0).map((_, i) => i))("Concurrent scrape #%i", async (i) => {
        ...
        const out = await (0, _1.scrapeURL)(id, url, types_1.scrapeOptions.parse({}));
