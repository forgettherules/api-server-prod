...
exports.crawlStatusWSController = void 0;
...
async function crawlStatusWS(ws, req) {
    ...
    const loop = async () => {
        ...
        const jobStatuses = await Promise.all(notDoneJobIDs.map(async (x) => [x, await (0, queue_service_1.getScrapeQueue)().getJobState(x)]));
...
async function crawlStatusWSController(ws, req) {
    try {
        ...
        logger_1.logger.error("Error occurred in WebSocket! (" + req.path + ") -- ID " + id + " -- " + verbose);
